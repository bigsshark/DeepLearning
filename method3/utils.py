...
def generator_batch_multitask(data_list, nbr_class_one=250, nbr_class_two=7, batch_size=32, return_label=True,
    ...
    while True:
        ...
        current_index = (batch_index * batch_size) % count
        
        #数据没有用完
        if count >= (current_index + batch_size):
            current_batch_size = batch_size
            batch_index += 1
        else:
            current_batch_size = N - current_index
        ...
        X_batch = np.zeros((current_batch_size, img_width, img_height, 3))
        Y_model_batch = np.zeros((current_batch_size, nbr_class_one))
        Y_color_batch = np.zeros((current_batch_size, nbr_class_two))

        for i in range(current_index, current_index + current_batch_size):
            line = data_list[i].strip().split(' ')
            #print line
            if return_label:
                label = (int(line[-2]),int(line[-1]))
            img_path = line[0]
            ...
            if return_label:
                Y_model_batch[i - current_index, label[0]] = 1
                Y_color_batch[i - current_index, label[1]] = 1
        ...
        if return_label:
            yield (X_batch,[Y_model_batch,Y_color_batch])
        else:
            yield X_batch
